...
extension SnapshotConfiguration where Input: UIView, Output == ImageBytes {

    /// Configuração padrão para snapshots de `UIView` como imagens.
    ///
    /// Observações:
    ///   - Usa valores padrão para precisão (`precision: 1`), layout (`sizeThatFits`), e outros parâmetros.
    ///   - Útil para casos onde a configuração básica atende às necessidades.
    public static var image: SnapshotConfiguration {
        return .image()
    }

    /// Cria uma configuração personalizada para snapshots de `UIView` como imagens.
    ///
    /// - Parameters:
    ...
    /// - Exemplo:
    ///   ```swift
    ///   let config = SnapshotConfiguration<UIView, ImageBytes>.image(
