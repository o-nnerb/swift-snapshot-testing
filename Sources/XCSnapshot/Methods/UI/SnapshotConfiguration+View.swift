...
extension SnapshotConfiguration where Input: SwiftUI.View, Output == ImageBytes {

    /// Configuração padrão para snapshots de `View` como imagens.
    ///
    /// Observações:
    ...
    /// Cria uma configuração personalizada para snapshots de `View` como imagens.
    ///
    /// - Parameters:
    ...
    /// - Exemplo:
    ///   ```swift
    ///   let config = SnapshotConfiguration<Text, ImageBytes>.image(
